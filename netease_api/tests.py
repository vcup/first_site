...
def create_album_data(s1, s2, s3):
...
def create_artist_data(al1, al2, al3):
...
def create_playlist_data(s1, s2, s3):
...
class SongModelTests(TestCase):
    ...
    def test_song_in_album(self):
        ...
        al1, al2, al3 = create_album_data(s1, s2, s3)

        self.assertEqual(list(s3.album_set.all()), [al3])
        self.assertEqual(list(s2.album_set.all()), [al2, al3])
        self.assertEqual(list(s1.album_set.all()), [al1, al2, al3])

    def test_song_in_artist(self):
        s1, s2, s3 = create_song_data()
        al1, al2, al3 = create_album_data(s1, s2, s3)
        a1, a2, a3 = create_artist_data(al1, al2, al3)

        self.assertEquals(list(s3.artist_set.all()), [a3])
        self.assertEquals(list(s2.artist_set.all()), [a2, a3])
        self.assertEquals(list(s1.artist_set.all()), [a1, a2, a3])

    def test_song_in_playlist(self):
        """反向查询拥有 Song 的 PlayList"""
        s1, s2, s3 = create_song_data()
        p1, p2, p3 = create_playlist_data(s1, s2, s3)

        self.assertEqual(list(s3.playlist_set.all()), [p3])
        self.assertEqual(list(s2.playlist_set.all()), [p2, p3])
        self.assertEqual(list(s1.playlist_set.all()), [p1, p2, p3])

    def test_song_in_user(self):
        s1, s2, s3 = create_song_data()
        p1, p2, p3 = create_playlist_data(s1, s2, s3)
        u1, u2, u3 = create_user_data(p1, p2, p3)

        self.assertEqual(list(s3.user_set.all()), [u3])
        self.assertEqual(list(s2.user_set.all()), [u2, u3])
        self.assertEqual(list(s1.user_set.all()), [u1, u2, u3])
